...
export class AleoService {
  ...
  /**
   * Verify an escrow proof
   * This verifies that:
   * 1. The proof format is valid
   * 2. The nullifier hasn't been used
   * 3. The escrow exists on-chain (if transaction ID provided)
   */
  async verifyEscrowProof(proof: EscrowProof): Promise<VerificationResult> {
    try {
      ...
      // If transaction ID is provided, verify it exists on-chain
      if (proof.transactionId) {
        const tx = await this.getTransaction(proof.transactionId);
        if (!tx) {
          return {
            valid: false,
  ...
  async verifyReputationProof(
    ...
    try {
      ...
      // If transaction ID is provided, verify on-chain
      if (proof.transactionId) {
        const tx = await this.getTransaction(proof.transactionId);
        if (!tx) {
          return {
            valid: false,
            error: 'Proof transaction not found on-chain',
          };
        }
      }
